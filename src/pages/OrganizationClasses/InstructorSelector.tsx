...
function useOrganizationInstructors() {
  ...
  const fetchInstructors = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select("instructor_uid, organization_uid, name, email, phone_number")
  ...
  const createInstructor = async (instructorData: IInstructorData) => {
    ...
    try {
      const instructorUid = crypto.randomUUID();
      const { error } = await supabase.from("organization_instructors").insert({
