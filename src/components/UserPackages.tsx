  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600">Manage and track your purchased class packages</p>
      </div>
      ...
      {userPackages.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-600 mb-4">Browse companies and purchase class packages to get started.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4">
          {userPackages.map((userPackage) => {
            ...
            return (
              <Card key={userPackage.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg">{userPackage.packages.name}</CardTitle>
                      <CardDescription>
                    ...
                    <div className="flex gap-2">
                      <Badge 
                        variant={
                          ...
                          status === 'used_up' ? 'secondary' : 'outline'
                        }
                      >
                        {status === 'active' ? 'Active' : 
                         status === 'expired' ? 'Expired' : 
                         status === 'used_up' ? 'Used Up' : 'Inactive'}
                      </Badge>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                    ...
                    {userPackage.packages.package_type === 'class_count' && (
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {userPackage.remaining_classes || 0} / {userPackage.packages.class_count} classes left
                        </span>
                      </div>
                    )}
                    {userPackage.packages.package_type === 'time_based' && userPackage.expires_at && (
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>Purchased {formatDate(userPackage.purchased_at)}</span>
                    </div>
                    <div className="flex items-center gap-2">
